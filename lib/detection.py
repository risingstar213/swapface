...
def detect_face(file_name):
    ...
    for index, face in enumerate(dets):
        ...
        image_data['face_' + str(index)] = face_data
        extract_img = img[top:bottom + 1, left:right + 1, :]
        cv2.imwrite(os.path.join('img\\extract',os.path.splitext(os.path.basename(file_name))[0] +'_'+ str(index) + '.jpg'), extract_img)
    base_name = os.path.splitext(os.path.basename(file_name))[0] + '.json'
    with open(os.path.join('json', base_name), 'w', encoding='utf-8') as f:
        f.write(json.dumps(image_data, indent=4))
    
    return img
if __name__ == '__main__':
    detect_face('img\\raw\\000076.jpg')